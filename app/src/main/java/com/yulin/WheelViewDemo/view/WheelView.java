...
public class WheelView extends View
{
		...
		for (ItemObject item : itemList)
		{
			if (item.isSelected())
			{
				if (onSelectListener != null)
					onSelectListener.endSelect(item.id, item);
				break;
			}
		}
	}
	...
	private void initData()
	{
		...
		for (int i = 0; i < dataList.size(); i++)
		{
			...
			itmItemObject.id = i;
			itmItemObject.itemText = dataList.get(i).itemText;
			itmItemObject.itemValue = dataList.get(i).itemValue;
			itmItemObject.isCanSelected = dataList.get(i).isCanSelected;
			itmItemObject.x = 0;
		...
		if (move > 0)
		{
			for (int i = 0; i < itemList.size(); i++)
			{
				if (itemList.get(i).isSelected())
				{
					...
					if (onSelectListener != null)
						onSelectListener.endSelect(itemList.get(i).id, itemList.get(i));
					break;
				}
			}
		} else
	...
	private void onSelectListener()
	{
		...
		for (ItemObject item : itemList)
		{
			if (item.isSelected())
			{
				onSelectListener.selecting(item.id, item);
			}
		}
	}
	 ...
	 * @param data
	 */
	public void setData(ArrayList<ItemDataObject> data)
	{
		this.dataList = data;
		initData();
		postInvalidate();
	 ...
	 * @param data
	 */
	public void resetData(ArrayList<ItemDataObject> data)
	{
		setData(data);
		invalidate();
	}
	...
	public void setDefault(int index)
	{
		if (index > itemList.size() - 1)
			return;
		if (itemList.get(index).isCanSelected)
		{
			float move = itemList.get(index).moveToSelected();
			defaultMove((int) move);
		} else
		{
			float move = itemList.get(index + 1).moveToSelected();
			defaultMove((int) move);
		}
	}
	...
	public interface OnSelectListener
	{
		 ...
		 * @param item
		 */
		public void endSelect(int id, ItemObject item);
		 ...
		 * @param item
		 */
		public void selecting(int id, ItemObject item);
